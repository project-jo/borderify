...
pub fn get_file(filename: &str, default_content: &str) -> std::fs::File {
  ...
  let dirpath = format!("{}\\.borderify", user_profile_path);
...
pub fn enable_startup() {
  ...
  if let Some(key) = get_registry_key() {
    if let Err(err) = key.set_value("borderify", &new_exe_path.to_string_lossy().to_string()) {
...
pub fn disable_startup() {
  ...
  if let Some(key) = get_registry_key() {
    match key.delete_value("borderify") {
      Ok(_) => {}
      Err(err) => {
